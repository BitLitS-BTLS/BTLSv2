...
uint256 hashGenesisBlock("0xc64208e2c2a13761b1ee44f6c643dfa9f978d30181208749e141f46097826387");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BitLitS: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BitLitS Signed Message:\n";
...
{
    // BitLitS: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BitLitS: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BitLitS: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        // Check proof of work
	// disabled check for protect fail sync wallets. will be returned back in the next versions when all its stable
...
{
    // BitLitS: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc64208e2c2a13761b1ee44f6c643dfa9f978d30181208749e141f46097826387");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1407775041;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085923552;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Bitlits: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitLitSMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BitLitSMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BitlitsMiner(CWallet *pwallet)
{
    printf("BitLitSMiner:\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitlits-miner");
    ...
    try { loop {
        ...
        printf("Running BitLitS with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BitLitSMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BitlitsMiner, pwallet));
}
